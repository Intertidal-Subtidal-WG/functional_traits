...
exposed_arrows1 <- mca_animal_exposed_end$vars_df %>%
  filter(Variable %in% c('benthic', 'motility_adult', 'epibiotic')) %>%
  `row.names<-`(c('Motile','Sessile','Not Benthic','Benthic','Not Epibiotic','Epibiotic'))
...
sheltered_arrows2 <- mca_animal_sheltered_end$vars_df %>%
  filter(Variable %in% c('benthic', 'motility_adult', 'epibiotic', 'scavenger', 'filter_suspension')) %>%
  `row.names<-`(c('Motile','Sessile','Not Filter-Suspension', 'Filter or Suspension feeder',
                  'Not a Scavenger', 'Scavenger', 'Not Benthic','Benthic','Not Epibiotic','Epibiotic')) %>%
...
ggplot(data.frame(mca_animal_sheltered_end$mcares$ind$coord), aes(x = Dim.1, y = Dim.2)) + 
    ...
    geom_text_repel(data = sheltered_arrows2, aes(x = Dim.1, y = Dim.2, 
                                     ...
                                     colour = Variable), seed = 3824) +
  labs(title = "Animals: Exposed", x = NULL, y = NULL) +
    scale_colour_manual(values = c('red','blue','purple', 'green', 'orange', 'blue')) +
