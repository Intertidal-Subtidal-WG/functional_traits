...
algae_test2 <- read_csv(here(traits, "20220314_1982-end_algae.csv")) %>%
  filter(intertidal == 'yes') 

algae_test_sheltered <- algae_test2 %>%
  filter(position == 'sheltered') %>%
  select(!(c(year, transect, position))) %>%
  distinct()

oldvnew <- comparedf(algae_test_sheltered, algae_sheltered, by = 'species')
#extract differences
diffs(oldvnew, what = c('observations'))
```
...
Note animal_all gives different result than any group of animals. I really don't understand why, it puts Astarias rubens in totally different locations.
...
fviz_cluster(testhcpc, geom = "point", main = "Factor map")
fviz_cluster(animal_allhcpc, geom = "point", main = "Factor map")
animal_all$ggclusterplot


fviz_dend(animal_allhcpc)
fviz_dend(testhcpc)

animal_all$clustmembers
animal_all$clustervariables
```
...
lavender grey, pale cerulean, glaucous, charcoal, metallic sunburst, coyote brown, dark brown, bistre

sets of 2 should be primarily drawn from the middle two of each part of the palette:
two blues
#94b4d8, #5484c4
two browns
#806238, #543414
