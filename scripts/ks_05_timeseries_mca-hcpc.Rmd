...
fviz_mca_var(mca_algae_sheltered_start$mcares, col.var = 'cos2', alpha.var = 'contrib',
                           gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
...
mca_algae_sheltered_start$vars_df

('branched_external_morphology', 'subtidal', 'leathery_external_morpohology', 'erect_morphology_growth_form', 'maximun_longevity', 'potential_asexual_reproduction')
```
...
exposed_arrows1 <- mca_algae_exposed_end$vars_df %>%
...
ggplot(data.frame(mca_algae_exposed_end$mcares$ind$coord), aes(x = Dim.1, y = Dim.2)) + 
  ...
  labs(title = "algaes: Exposed", x = NULL, y = NULL) +
...
ggsave(filename = here(plots, '20220323_algae_exposed.png'), device = 'png', 
       height = 5, width = 7, units = 'in')
...
sheltered_arrows2 <- mca_algae_sheltered_end$vars_df %>%
  filter(Variable %in% c('branched_external_morphology', 'subtidal', 'leathery_external_morphology', 'erect_morphology_growth_form')) %>%
  `row.names<-`(c('Intertidal Only','Also found in Subtidal','Not Erect Growth', 'Erect growth',
                  'Not Leathery', 'Leathery', 'Not Branched', 'Branched')) %>%
...
ggplot(data.frame(mca_algae_sheltered_end$mcares$ind$coord), aes(x = Dim.1, y = Dim.2)) + 
    ...
    geom_text_repel(data = sheltered_arrows2, aes(x = Dim.1, y = Dim.2, 
                                     ...
                                     colour = Variable), seed = 3824) +
  labs(title = "Algae: Sheltered 2010-present", x = NULL, y = NULL) +
    scale_colour_manual(values = c('#806238','#94b4d8','#5484c4', '#374658', 'orange', 'blue')) +
...
ggsave(filename = here(plots, '20220323_algae_sheltered-end.png'), device = 'png', 
       height = 5, width = 7, units = 'in')  
  
sheltered_arrows1 <- mca_algae_sheltered_start$vars_df %>%
  filter(Variable %in% c('branched_external_morphology', 'subtidal', 'leathery_external_morpohology',
                         'erect_morphology_growth_form', 'maximun_longevity', 'potential_asexual_reproduction')) #%>%
  #`row.names<-`(c('Motile','Sessile','Not Epibiotic','Epibiotic')) %>%
...
ggplot(data.frame(mca_algae_sheltered_start$mcares$ind$coord), aes(x = Dim.1, y = Dim.2)) + 
    ...
    geom_text_repel(data = sheltered_arrows1, aes(x = Dim.1, y = Dim.2, 
                                     ...
                                     colour = Variable), seed = 3824) +
  labs(title = "algaes: Sheltered 1982-1994", x = NULL, y = NULL) +
...
ggsave(filename = here(plots, '20220323_algae_sheltered-start.png'), device = 'png', 
       height = 5, width = 7, units = 'in')
```
...
#added scavenger and filter/suspension to >.6 cos2
# make subset of vars to plot arrows for
exposed_arrows1 <- mca_animal_exposed_end$vars_df %>%
  filter(Variable %in% c('motility_adult', 'epibiotic')) %>%
  `row.names<-`(c('Motile','Sessile','Not Epibiotic','Epibiotic'))
...
ggplot(data.frame(mca_animal_exposed_end$mcares$ind$coord), aes(x = Dim.1, y = Dim.2)) + 
    ...
    geom_text_repel(data = exposed_arrows1, aes(x = Dim.1, y = Dim.2, 
                                     label = rownames(exposed_arrows1), 
                                     colour = Variable), seed = 3824) +
  labs(title = "Animals: Exposed", x = NULL, y = NULL) +
    scale_colour_manual(values = c('#806238','#5484c4','purple')) +
    geom_segment(data = exposed_arrows1, aes(x = 0, y = 0, xend = Dim.1, yend = Dim.2), 
               arrow = arrow(length = unit(0.2, "cm")), colour = "black") +
  xlim(-1.5,1.6) +
...
ggsave(filename = here(plots, '20220323_animal_exposed.png'), device = 'png', 
       height = 5, width = 7, units = 'in')


# make subset of vars to plot arrows for
sheltered_arrows2 <- mca_animal_sheltered_end$vars_df %>%
...
ggsave(filename = here(plots, '20220323_animal_sheltered-end.png'), device = 'png', 
       height = 5, width = 7, units = 'in')  
...
ggsave(filename = here(plots, '20220323_animal_sheltered-start.png'), device = 'png', 
       height = 5, width = 7, units = 'in')
```
