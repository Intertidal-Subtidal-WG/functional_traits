...
trait_raw <- read.csv(here("data", '20220312_ShoalsFunctionalTraits.csv'), 
                      na.strings = c("","NA", 'N/A'))

#or load in imputed data to skip ahead
...
algae_raw <- trait_raw %>%
  subset(group == 'Algae') %>%
    dplyr:::select(species, group, common_name_division_bgr,
                   turf_subcanopy_canopy_algae, #steneck_dethier_morphology_algae, # note 20220117 column names in lowercase
                   macrophyte, leathery, corticated, foliose,
                   crustose, articulated, calcareous,
                   body_size_avg_bin, benthic, epibiotic,
                   #epiphytic, epizoic, epifaunal, epilithic, 
                   #invasive, #morphology1
                 intertidal, subtidal)
...
animal_raw <- trait_raw %>%
  ...
  dplyr:::select(species, group, common_name_division_bgr, body_size_avg_bin,
                 #morphology1,
                 calcareous,
                 dietary_pref_c, trophic_level, motility_juv,
...
# save
write_csv(algae_clean, here("data","20220312_algae_clean.csv"))
write_csv(animal_clean, here("data","20220312_animal_clean.csv"))
```
...
write.csv(algae_imp2, file = here('data', 'clean','20220312_algae_imputed.csv'))
...
animal_imp <- mice(animal_clean[,3:ncol(animal_clean)], m=5, method = 'cart', print = F)
...
write.csv(animal_imp2, file = here('data', 'clean','20220312_animal_imputed.csv'))
...
algae_dataSET <- algae_imp2[,c(1:10)]
...
write.csv(algae_matrix2, file = here('data', 'clean', '20220117_algae_matrix.csv'))
```
...
write.csv(animal_matrix2, file = here('data', 'clean', '20220117_animal_matrix.csv'))
```

```{r export graphs}
png(filename = here('output', '20220117_algae-dissimilarity.png'),
...
png(filename = here('output', '20220117_animal-dissimilarity.png'),
    width = 1000, height = 500,
    units = 'px')
coldiss(animal_matrix)
dev.off()
...
algae_clust <- cutree(algae_fit, 5)  # change number of clusters
...
ggsave(here('output', '20220117_algae-cluster-dendro.png'), plot = last_plot())
...
ggsave(here('output', '20220117_animal-cluster-dendro.png'), plot = last_plot())
```
...
tsne_algae <- ggplot(tsne_plot_ag, aes(x=x, y=y)) + 
  ...
  #geom_point(aes(color = turf_subcanopy_canopy_algae)) +
  geom_point(aes(color = steneck_dethier_morphology_algae)) +
  theme_classic()

tsne_algae

ggsave(here('output', '20220117_algae-cluster-tsne_sd.png'), plot = last_plot())
...
tsne_animal <- ggplot(tsne_plot_an, aes(x=x, y=y)) + 
  ...
  #geom_point(aes(color = trophic_level)) +
  #geom_point(aes(color = dietary_pref_c)) +
  geom_point(aes(color = body_size_avg_bin)) +
  theme_classic()

tsne_animal

ggsave(here('output', '20220117_animal-cluster-tsne_diet.png'), plot = last_plot())
