...
animal_raw[sapply(animal_raw, is.character)] <- lapply(animal_raw[sapply(animal_raw, is.character)], 
                                       as.factor)
...
animal_imp <- mice(animal_raw[,3:ncol(animal_raw)], m=5, method = 'polyreg', print = F)
...
algae_dataSET <- algae_imp2[,c(1:12)]
...
#Select the variables to use from the imputed data
animal_dataSET <- animal_imp2[,c(1:18)]
