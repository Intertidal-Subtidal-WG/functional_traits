...
algae_raw <- trait_raw %>%
  ...
  dplyr::select(species, group, common_name_division_bgr,
                ...
                #epiphytic, epizoic, epifaunal, epilithic, #invasive, 
                intertidal, subtidal)

animal_raw <- trait_raw %>%
  filter(group %in% c('Invertebrate', 'Vertebrate')) %>%
...
algae_imp <- mice(algae_clean[,3:ncol(algae_raw)], m=5, method = 'logreg', print = F)
algae_imp2 <- complete(algae_imp) #action
algae_imp2 <- cbind(algae_clean[,1:2],algae_imp2)
...
animal_imp <- mice(animal_clean[,3:ncol(animal_raw)], m=5, method = 'polyreg', print = F)
animal_imp2 <- complete(animal_imp) #action
animal_imp2 <- cbind(animal_clean[,1:2],animal_imp2)
...
tsne_obj <- Rtsne(algae_matrix, is_distance = TRUE, perplexity = 5)
plot(tsne_obj$Y)
```
