...
animal_imp2 <- read.csv(file = here('data', 'clean','20210806_animal_imputed.csv'))[,-1]
algae_imp2 <- read.csv(file = here('data', 'clean','20210806_algae_imputed.csv'))[,-1]
...
## Step 1.5, filter to specieslists if needed
```{r}
sp_list <- read.csv(here('data', 'fielddata', 'sp_1982.csv'))
...
```{r}
# make backup while troubleshooting
...
algae_eval <- algae_eval %>%
    na_if('NaN') %>%
  select(!(c(WCM, WCE, BK))) %>% # these columns are always NaN for some reason?
...
for(i in 1:length(nomclustEvals)){
  ...
  ggsave(EvalPlot, device = "pdf",
         filename = here('output', 'cluster_ks', paste0("algae_",nomclustEvals[i],"_plot.pdf")))
}
...
ggsave(EvalPlotPSFE, filename = here('output', 'cluster_ks', 'Algae_EvalPlotPSFE.pdf'),
       width = 9, height = 4)
...
ggsave(EvalPlotPSFM, filename = here('output', 'cluster_ks', 'Algae_EvalPlotPSFM.pdf'), 
       width = 9, height = 4)
...
ggsave(EvalPlotAIC, filename = here('output', 'cluster_ks', 'Algae_EvalPlotAIC.pdf'), 
       width = 9, height = 4)
...
ggsave(EvalPlotBIC, filename = here('output', 'cluster_ks', 'Algae_EvalPlotBIC.pdf'), 
       width = 9, height = 4)
...
for(i in 1:length(nomclustEvals)){
  ...
  EvalPlot <- plotEvals(EvalData, nomclustEvals[i])
  ggsave(EvalPlot, device = "pdf",
         filename = here('output', 'cluster_ks',  paste0("animal_", nomclustEvals[i],"_plot.pdf")))
}
...
ggsave(EvalPlotPSFE, filename = here('output', 'cluster_ks', 'Animal_EvalPlotPSFE.pdf'),
       width = 9, height = 4)
...
ggsave(EvalPlotPSFM, filename = here('output', 'cluster_ks', 'Animal_EvalPlotPSFM.pdf'), 
       width = 9, height = 4)
...
ggsave(EvalPlotAIC, filename = here('output', 'cluster_ks', 'Animal_EvalPlotAIC.pdf'), 
       width = 9, height = 4)
...
ggsave(EvalPlotBIC, filename = here('output', 'cluster_ks', 'Animal_EvalPlotBIC.pdf'), 
       width = 9, height = 4)
...
```{r export graphs}
png(filename = here('output', 'cluster_ks', '20210910_algae-dissimilarity.png'),
...
png(filename = here('output', 'cluster_ks', '20210910_animal-dissimilarity.png'),
    width = 1000, height = 500,
    units = 'px')
coldiss(animal_matrix)
dev.off()
...
set number of clusters based on previous evaluations
...
algae_clust <- cutree(algae_fit, 12)  # change number of clusters
...
ggsave(here('output', 'cluster_ks', '20210830_algae-cluster-dendro.png'), plot = last_plot())

######Animals
animal_fit <- hclust(animal_matrix, method = methodsClust[3])  #change cluster method
animal_clust <- cutree(animal_fit, 3)  # change number of clusters
...
ggsave(here('output', '20210910_animal-cluster-dendro.png'), plot = last_plot())
```
...
ggsave(here('output', 'cluster_ks', '20210830_algae-cluster-tsne2.png'), plot = last_plot())
...
ggsave(here('output', 'cluster_ks', '20210910_animal-cluster-tsne_1982.png'), plot = last_plot())
