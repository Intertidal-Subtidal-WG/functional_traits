...
#   #Extract the validation metrics
   good_eval <- melt(good_fit$eval, id.vars = "names")
...
# 
#   #Extract the validation metrics
  good2_eval <- melt(good_fit$eval, id.vars = "names")
...
# 
   evaluation <- rbind(good_eval, good2_eval,
...
save(evaluation, file = here('output', 'cluster_ks', 'algae_evaluation_2-25.RData'))

algae_eval <- evaluation
```

scale, prepare for evaluation

```{r}
algae_eval$distance <- as.factor(algae_eval$distance)

#remove NAs and cluster names
...
#set as numeric
algae_eval$value <- as.numeric(algae_eval$value)

#scale
algae_eval$value_scaled <- algae_eval$value/max(algae_eval$value)

unique(algae_eval$L1)
...
save(evaluation, file = here('output', 'cluster_ks', 'animal_evaluation_2-25.RData'))

animal_eval <- evaluation
```


```{r}
animal_eval$distance <- as.factor(animal_eval$distance)
...
#set as numeric
animal_eval$value <- as.numeric(animal_eval$value)
